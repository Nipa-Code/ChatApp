from chatapp.backend.ratelimits.limit import request_is_limited
...
def limit(endpoint: str, limits: int = 20, period: int = 10):
    """Ratelimit decorator"""
    ...
    def decorator_func(func):
        ...
        async def wrapper(*args, **kwargs):
            if await request_is_limited(
                redis, "admin", limits, timedelta(seconds=period)
            ):
                print(f"limited request ")
                data = "You are being ratelimited"
                return JSONResponse(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                    content=f"{data}. Response status: {status.HTTP_429_TOO_MANY_REQUESTS}",
                )
            # process request
            else:
...
class __BucketBase:
    ...
    async def _pre_call(
        self, _request_id: int, request: fastapi.Request, *args, **kwargs
    ) -> None:
        ...
        self.state.update(
            {
                request_id: self._StateVariables(
                    remaining_requests=None, clean_up_tasks=[]
                )
            }
        )
    ...
    def __call__(self, *args):
        ...
        function_hash = hashlib.md5(
            inspect.getsource(route_callback).encode("utf8")
        ).hexdigest()
        ...
        @router.head(
            "/" + route_callback.__name__, name=f"{route_callback.__name__} rate limit"
        )
        ...
        async def caller(*_args, **_kwargs) -> typing.Union[JSONResponse, Response]:
            ...
            bypass = (
                await self.BYPASS()
                if inspect.iscoroutinefunction(self.BYPASS)
                else self.BYPASS()
            )
            if not bypass:
                try:
                    ...
                    response = JSONResponse(
                        content={
                            "message": "You are currently on cooldown. Try again later."
                        },
                        status_code=429,
                    )
                    ...
                    log.error(
                        "Failed to increment rate limiter, falling back to 500.",
                        exc_info=e,
                    )
                    response = JSONResponse(
                        content={
                            "message": "Unknown error occurred, please contact staff."
                        },
                        status_code=500,
                    )
    ...
    async def get_remaining_requests(self, request_id: int) -> int:
        ...
        if state.remaining_requests is None:
            state.remaining_requests = await self._calculate_remaining_requests(
                request_id
            )
...
class UserRedis(__BucketBase):
    ...
    async def _record_interaction(self, request_id: int) -> None:
        ...
        log.debug(
            f"Recorded interaction of user {self.state[request_id].user_id} on {self.ROUTE_NAME}."
        )
    ...
    async def _calculate_remaining_requests(self, request_id: int) -> int:
        ...
        log.debug(
            f"Remaining interactions of user {self.state[request_id].user_id} on {self.ROUTE_NAME}: {remaining}."
        )
        return remaining
